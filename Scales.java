...
public class Scales {
	//static double SCALEX = 0.5, SCALEY = 2.0;
	...
	static MyImage execute(MyImage input, double scalex, double scaley) {
		...
		for(i = 0; i < height2; i++) {
			for(j = 0; j < width2; j++) {
				...
				x1 = calcX(j, i, width1, height1, scalex, scaley);
				y1 = calcY(j, i, width1, height1, scalex, scaley);
	...
	static double calcX(int x2, int y2, int width1, int height1, double scalex, double scaley) {
		double x = 0.0;
	...
	static double calcY(int x2, int y2, int width1, int height1, double scalex, double scaley) {
		double y = 0.0;
